export class TintimWebhookController {
    ...
    async buscarLeadComTentativas(telefone) {
        ...
        for (let i = 0; i < delays.length; i++) {
            ...
            if (i < delays.length - 1) {
                console.log(`🕒 Tentativa ${i + 1} de ${delays.length} - Aguardando ${delays[i]}ms...`);
    ...
    async atualizarFiledsWebhookTintim(req, res) {
        ...
        const { campaing_name = "sem informação", adset_name = "sem informação", ad_name = "sem informação", } = webhookData?.ad || {};
        ...
        const camposPersonalizados = await this.clienteModel.buscarIdsPorNomesCampos(camposNames);
        const body = {
