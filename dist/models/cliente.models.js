...
export class ClienteModel {
    ...
    async buscarUsuarioPorNome(nome) {
        try {
            ...
            console.error("Erro ao buscar usuário por nome:", error.response?.data || error);
            return null;
        }
    }
    async buscarLeadPorTelefone(telefone) {
        try {
    ...
    async buscarIdsPorNomesCampos(campos) {
        try {
            ...
            const camposEncontrados = response.data._embedded.custom_fields
                .filter((campo) => camposMap.has(campo.name)) // Verifica se o campo existe na lista
                .map((campo) => {
                ...
                if (campo.enums && enumNome) {
                    const enumEncontrado = campo.enums.find((e) => e.value === enumNome);
    ...
    async adicionarTask({ text, leadId, responsible_user_id, result_text, complete_till = Math.floor(Date.now() / 1000), }) {
        try {
            const taskData = [
                {
                    text,
    ...
    async adicionarNota({ leadId, text, }) {
        try {
            const noteData = [
                {
                    entity_id: leadId,
